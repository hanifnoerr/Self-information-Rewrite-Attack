...
In Colab, choose **Runtime > Change runtime type > L4 GPU**, add the approved
Hugging Face token as a Colab Secret named `HF_TOKEN`, and run the cells from
top to bottom. The notebook starts with 10 KGW samples and copies final outputs
to Google Drive.

The browser notebook currently uses the released-code
`meta-llama/Llama-3.2-3B-Instruct` checkpoint for the SIRA-Tiny reproduction
attempt. Adapted SIRA-Small uses public `Qwen/Qwen2.5-7B-Instruct` in 4-bit NF4
until access to the separate `meta-llama/Meta-Llama-3-8B-Instruct` repository is
available. The notebook also enables the LLM Base64 baseline.

`RESET_OUTPUTS = True` is enabled by default. This prevents the official
resume logic from silently reusing attack outputs generated by a previously
selected model.
...
```text
/content/sira_outputs/base64_llm/base64_side_by_side.jsonl
/content/sira_outputs/base64_llm/base64_side_by_side.csv
```

These show, for every sample:

- original watermarked text, raw Base64, and decoded original text;
- direct LLM-generated Base64 and its decoded result/error;
- normal paraphrase;
- that same normal paraphrase externally encoded and decoded.

The final control is important: if the normal paraphrase and its externally
encoded/decoded copy have the same detector result, Base64 itself added no
attack effect.
